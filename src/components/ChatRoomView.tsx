  ...
  return (
    <div className="h-[calc(100vh-60px)] flex flex-col bg-zinc-950 text-white select-text">
      ...
      <div className="border-b border-zinc-800 bg-zinc-900/90 px-4 py-2.5 flex items-center justify-between gap-3 shrink-0">
        ...
        {/* Right Tools: Switch identity, Members, Share, Admin */}
        <div className="flex items-center space-x-1.5 shrink-0">
          {/* Identity Switcher Dropdown (Multi-User Simulation) */}
          <div className="hidden sm:flex items-center space-x-1 bg-zinc-950 border border-zinc-800 rounded-lg p-1 text-xs">
            <UserCheck className="w-3.5 h-3.5 text-zinc-400 ml-1.5" />
            <span className="text-zinc-400 text-[11px] hidden md:inline">Speaking as:</span>
            <select
              value={currentMember.session_id}
              onChange={(e) => {
                const target = members.find((m) => m.session_id === e.target.value);
                if (target) onSwitchIdentity(target);
              }}
              className="bg-transparent text-white font-medium text-xs focus:outline-none cursor-pointer pr-1"
            >
              {members.map((m) => (
                <option key={m.session_id} value={m.session_id} className="bg-zinc-900 text-white">
                  {m.username} ({m.role})
                </option>
              ))}
            </select>
      ...
      {/* Identity Banner for Mobile */}
      <div className="sm:hidden px-4 py-1.5 bg-zinc-900/60 border-b border-zinc-800/80 flex items-center justify-between text-xs">
        <div className="flex items-center space-x-2">
        ...
        <select
          ...
          className="bg-zinc-800 text-zinc-200 rounded px-2 py-0.5 text-[11px] focus:outline-none"
        >
          {members.map((m) => (
            <option key={m.session_id} value={m.session_id}>
              Switch to {m.username}
            </option>
          ))}
        </select>
      </div>
